...
Như vậy, nó sẽ chạy lệnh `e = add(a, b)` sau đó lưu kết quả vào biến `e`, kéo theo trạng thái chương trình thay đổi.
...
Nếu điều này xảy ra, giả sử, trên một GPU (hay thậm chí nhiều GPU), bộ thông dịch Python có thể gây ?
...
Cơ chế này được sử dụng trong nhiều framework, gồm Theano, Keras và TensorFlow (hai cái sau đã hỗ trợ lập trình mệnh lệnh).
Quy trình thường gồm những bước sau:
...
Thứ hai, bộ biên dịch có thể tối ưu và viết lại mã nguồn thành `print((1 + 2) + (3 + 4))` hoặc thậm chí `print(10)`.
Điều này hoàn toàn có thể bởi bộ biên dịch có thể thấy toàn bộ mã nguồn rồi mới dịch sang mã máy.
...
Để hiểu rõ hơn vấn đề ta xem xét mô phỏng về lập trình mệnh lệnh
